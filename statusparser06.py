...
def dictgenerator(statusfile_list):  # outputs basedict
    ...
    for line in statusfile_list:
        ...
        if line.startswith(' ', 0, 2) and not line.startswith(' /', 0, 3):
            ...
            basedict[packname_list[1]]['Description'] = descr_list
    return basedict
...
def pagecreator(basedict):
    ...
    for directory, value in basedict.items():
        ...
        for dkey in value:
            if dkey == 'Rdepends':
                for i in value[dkey]:
                    htmlrdepnd += '<li><a href="https://ratracemaverick.com/parse/' + parent_dir + i + '/' + i + '.html">' + i + '</a>\n<br></li>'
        if len(htmlrdepnd) <= 2:
